...
- **Slash commands** — `/help`, `/server`, `/server-add`, `/t2i-workflow`
...
| `ARCHIVE_VOLUME` | *(empty)* | Host path to the volume for `/archive-*` |
...
| `/server-add <name> <host:port:os>` | Add a server (`os`: `unix` or `windows`)                                               |
...
| `/archive-session [name]`          | Copy this session's images into the archive volume, then delete the originals           |
| `/archive-today [name]`            | Archive images generated today into the archive volume                                  |
